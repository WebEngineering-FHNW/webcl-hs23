...
const dartboard = (canvas, model) => {
    ...
    const pieSlice = (start, end, radius, color) => {
        ctx.beginPath();
    ...
    const pieFrame = (start, end, radius, color = "silver") => {
        ctx.beginPath();
        ctx.moveTo(centerx, centery);
        ctx.arc(centerx, centery, radius-2, adjust(start), adjust(end), clockwise);
    ...
    const paint = () => {
        ...
        let   start     = 0;
        const increment = 1 / model.values.length;
...
const updateModelFromEvent = (dartView, evt, model) => {
    const relativeX = evt.offsetX; // selection position via mouse or touch where 0,0 is the canvas top left corner
    const relativeY = evt.offsetY;
    // normalize into cartesian coords where 0,0 is at the center of a unit circle
    const y = 2 * (((dartView.height / 2) - relativeY) / dartView.height);
    const x = 2 * (relativeX / dartView.width - 0.5);
